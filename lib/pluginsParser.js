    ...
    var plugins = _.reduce( pkg.dependencies, function( result,
      ...
      if( pluginConfigs ){
        ...
        pluginConfigs = _.reduce( pluginConfigs, function( memo,
          ...
          var satisfies = false;
          try{
            satisfies = verifyPluginConfig( pluginConfig, modulePkg.version );
          } catch( err ) {
          ...
          if( !satisfies ){
            log( chalk.magenta( "Plugin " + pluginConfig.name
              + " is not compatible with d-pac framework v" + modulePkg.version ) );
            if( pluginConfig.compatibility ){
              log( chalk.magenta( "(requires d-pac framework " + pluginConfig.compatibility + ")" ) );
            }
          } else {
