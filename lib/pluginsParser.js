    ...
    var plugins = _.reduce( pkg.dependencies, function( result,
      ...
      if( pluginConfigs ){
        ...
        pluginConfigs = _.reduce( pluginConfigs, function( memo,
          ...
          var satisfies = false;
          try{
            satisfies = verifyPluginConfig( pluginConfig, pkg.version );
          } catch( err ) {
            if( err instanceof assert.AssertionError ){
              console.log( chalk.red( "Error in " + ( pluginConfig.name || "unidentified" ) + " plugin:" ), err.message );
              return memo;
            } else {
              throw err;
            }
          ...
          if( !satisfies ){
            console.log( chalk.red( "Plugin " + pluginConfig.name
              + " is not compatible with d-pac framework v" + pkg.version ) );
            if( pluginConfig.compatibility ){
              console.log( chalk.red( "(requires d-pac framework " + pluginConfig.compatibility + ")" ) );
            }
          } else {
